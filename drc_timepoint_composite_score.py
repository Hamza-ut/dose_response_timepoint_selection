...
def timepoint_composite_score(df, group_fields, dose_field, od_field, time_field, standardize=False, method='minmax'):
    """
    Parameters:
    ...
    mean_df = df.groupby(group_fields + [dose_field, time_field])[od_field].agg([
        ('mean_od', 'mean'),
        ('std_od', 'std'),
        ('count', 'count')
    ]).reset_index()
    ...
    results_df['composite_score'] = sum(results_df[f'{metric}_norm'] * abs(weight) for metric, weight in weights.items())
    
    # Select best timepoint for each group based on composite score
    best_timepoints_df = results_df.loc[results_df.groupby(group_fields)['composite_score'].idxmax()]
    
    # Select columns to keep dynamically
    columns_to_keep = group_fields + [time_field] + ['snr', 'correlation', 'coeff_of_variation', 'dynamic_range', 'smoothness', 'composite_score']
...
def main():
    ...
    config_file = input("Please enter the path to your configuration file (e.g., 'config_sf.json'): ")
    try:
        ...
        timepoint_using_composite_score = timepoint_composite_score(df, GROUP_FIELDS, DOSE_FIELD, OD_FIELD, TIME_FIELD, standardize=True, method='minmax')
